...
/**
 * Moteur de calcul alpha-bêta mutualise entre l'IA et les threads d'evaluation.
 */
public class AlphaBetaEngine extends  IAEngine {
    public AlphaBetaEngine(Couleur couleurIA) {
        super(couleurIA);
    }
    ...
    public int evaluerCoup(Plateau plateau, Coup coup, int profondeur) {
        ...
        return alphabeta(prochainEtat, profondeur - 1, couleurSuivante(couleurIA), Integer.MIN_VALUE, Integer.MAX_VALUE);
    }

    private int alphabeta(Plateau plateau, int profondeur, Couleur joueurCourant, int alpha, int beta) {
        ...
        for (Coup coup : coups) {
            Plateau prochainEtat = plateau.copy();
            appliquerCoup(prochainEtat, coup, true);
            int valeurCoup = alphabeta(prochainEtat,profondeur - 1,couleurSuivante(joueurCourant),alpha,beta);
